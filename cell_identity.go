...
// a slice, a map, or anything containing one. It is what KeyOf returns for
...
// change. Build one with KeyOf rather than by hand.
...
// KeyOf returns a value usable as a map key for v.
//
// A value Go can compare is returned as it is, so counter[1] and counter["a"]
// read a Counter result directly. A value Go cannot compare comes back as an
// UncomparableKey, so counter[insyra.KeyOf(blob)] reads the count of a cell
// that could not otherwise be a key at all.
