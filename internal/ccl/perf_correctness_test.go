...
func TestFoldingLeavesAFailingAggregateAlone(t *testing.T) {
	ctx := perfCtx(t, 5)
	node, err := CompileExpression("A / SUM(Z)")
	if err != nil {
		t.Fatal(err)
	}
	// Z is past the last column, so the aggregate cannot be evaluated; the
	// tree must survive untouched rather than swallowing the problem.
	folded := FoldRowInvariantAggregates(node, ctx)
	if folded != node {
		t.Error("a failing aggregate was rewritten")
	}
...
// evalRowsErr is evalRows that reports the first per-row error instead of
// failing the test, so a folded and an unfolded run can be compared on errors
// as well as values.
func evalRowsErr(t *testing.T, ctx *MapContext, expr string, fold bool) ([]any, error) {
	t.Helper()
	node, err := CompileExpression(expr)
	if err != nil {
		t.Fatalf("compile %q: %v", expr, err)
...
// IF evaluates only the branch it selects. Folding evaluates an aggregate up
// front, so an aggregate in a branch no row selects must neither start failing
// the expression nor change the values of the rows that are selected.
func TestFoldingKeepsIFBranchSemantics(t *testing.T) {
	ctx := mapCtx(t, map[string][]any{"a": {1.0, 2.0, 3.0}, "b": {0.0, 5.0, -1.0}})
	for _, expr := range []string{
		"IF(A > 0, A, STDEV(Z))",      // failing aggregate, never selected
		"IF(A > 0, A, SUM(A) / 0)",    // aggregate folds, the failure is never selected
		"IF(B > 0, SUM(A), STDEV(Z))", // failing aggregate selected on two rows
		"IF(B > 0, A / SUM(B), 0)",
	} {
		t.Run(expr, func(t *testing.T) {
			unfolded, uerr := evalRowsErr(t, ctx, expr, false)
			folded, ferr := evalRowsErr(t, ctx, expr, true)
			if (uerr == nil) != (ferr == nil) {
				t.Fatalf("folding changed whether it fails: unfolded %v, folded %v", uerr, ferr)
			}
			if uerr != nil && uerr.Error() != ferr.Error() {
				t.Fatalf("folding changed the error: unfolded %v, folded %v", uerr, ferr)
			}
			for i := range unfolded {
				if !sameValue(unfolded[i], folded[i]) {
					t.Fatalf("row %d: folded %v, unfolded %v", i, folded[i], unfolded[i])
