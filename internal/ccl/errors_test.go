...
func TestCompileErrorPositionIsAByteOffset(t *testing.T) {
	// "(A B)": the third token is B, whose byte offset is 3. A token index
	// would be 2, which points at a space.
	_, err := CompileExpression("(A B)")
	var ce *CompileError
	if !errors.As(err, &ce) {
		t.Fatalf("error is %T, want *CompileError", err)
	}
	if ce.Offset != 3 {
		t.Errorf("Offset = %d, want 3 (the byte offset of B, not its token index)", ce.Offset)
	}
}

// The statement path used by ExecuteCCL reports the same way, and keeps each
// statement's source text for runtime failures.
func TestCompileMultilineStatementsKeepsSource(t *testing.T) {
	stmts, err := CompileMultilineStatements("NEW('x') = A + 1; NEW('y') = A * 2")
	if err != nil {
		t.Fatal(err)
	}
	if len(stmts) != 2 || stmts[0].Src != "NEW('x') = A + 1" || stmts[1].Src != "NEW('y') = A * 2" {
		t.Fatalf("statements = %+v", stmts)
	}

	_, err = CompileMultilineStatements("NEW('x') = A + 1\nNEW('y') = (A B)")
	var ce *CompileError
	if !errors.As(err, &ce) {
		t.Fatalf("error is %T, want *CompileError", err)
	}
	if ce.Expr != "NEW('y') = (A B)" {
		t.Errorf("Expr = %q, want the failing statement", ce.Expr)
	}
}
...
func TestErrorMessagesHaveNoGoInternals(t *testing.T) {
	...
	for _, expr := range []string{"1 +)", "(A B)", "A:1", "1:B", "(1+2):(3+4)"} {
