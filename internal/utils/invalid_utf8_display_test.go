...
// A string cell can hold bytes that are not text. Quoting those bytes and
// handing them to a terminal makes the cell unreadable and, worse, knocks the
// column out of alignment: no width calculation can be right for bytes that
// are not text.
