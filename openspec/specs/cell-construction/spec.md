# cell-construction Specification

## Purpose
Passing a slice to a list constructor is ambiguous: it can mean "these are my values" or "this is one value". This capability fixes which one it means by default and how to say the other. The default is to spread, because that is what makes building a list read like building a Series; the marker is how a caller keeps one value whole, for one argument, without leaving the call. It exists because the only way to do that used to be a different method that could not be mixed into the same call, and nothing said the two disagreed.
...
- **WHEN** 以被標記的值呼叫 `Update`、`InsertAt`、`ReplaceAll`、`ReplaceNilsWith`、`Shift` 的填補值、`UpdateElement` 或 `DataTable.Replace`
- **THEN** 格子中存放的是原值，不是標記
