# parquet-read-column-limit Specification

## Purpose
Makes `parquet.ReadColumnOptions.MaxValues` a real guard: `ReadColumn` checks the selected row groups' row count from file metadata and refuses the read before loading anything when it exceeds the limit.
