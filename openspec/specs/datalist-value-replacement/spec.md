# datalist-value-replacement Specification

## Purpose
Defines which cells `DataList.ReplaceFirst`, `ReplaceLast`, and `ReplaceAll` may touch: only cells equal to `oldValue`, with NaN matched only when `oldValue` is NaN. Exists because `ReplaceLast` used to rewrite a trailing NaN in place of the requested value.
