...
## Purpose
A call that locks or snapshots data either protects what it says it protects or does not compile. AtomicDoAll locks every value it is given, GroupBy aggregates the table it grouped rather than whatever the parent holds later, and ExecuteCCL leaves the table either fully updated or untouched.
