# datafetch-twstock-adjusted Specification

## Purpose
`TWStock` 的除權息與還原價：`ExRights` 取證交所指定期間的除權除息計算結果表（含交易所自己的調整因子，超過一年分頁），`DailyPricesAdjusted` 以 Yahoo `Adj Close` 相同的向後調整慣例產出 `AdjFactor` 與調整後 OHLC，使報酬序列在除權息日不出現假跌幅；櫃買中心無歷史端點，`TWMarketTPEx` 回明確錯誤。
