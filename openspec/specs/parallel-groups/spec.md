...
## Purpose
What a group, a run, a result slot and a worker failure are in `insyra/parallel`: a group is a reusable list of functions, each run keeps its own results and can only be awaited, and a function that panics or cannot be called is reported as a `WorkerError` rather than mixed into the results.
## Requirements
