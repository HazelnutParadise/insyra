...
含 NaN／±Inf 的 DataList 與 DataTable SHALL 能 `SaveState` 並 `RestoreVariables` 回相同型別與值（NaN 對 NaN）。不含 NaN／±Inf 的變數 SHALL 以與先前版本相同的格式寫入（DataTable 仍為 JSON 字串）；舊格式 state.json SHALL 仍可讀。
...
- **THEN** state.json 中該變數的 `data` 等於 `ToJSON_String(true)` 的字串
