# quant-bootstrap Specification

## Purpose
`quant` 的機率式預測：以 block bootstrap（moving block 或 stationary）對報酬序列整塊重抽樣，產生可由 seed 重現的模擬報酬與權益路徑，並以 R type-7 分位數在每個時點取百分位帶供扇形圖使用。輸入驗證採拒絕而非補零。
