# ccl-error-reporting Specification

## Purpose
A failed CCL expression tells the caller which question to ask next: whether the formula is malformed or the data is, where in the expression text the problem is, and which row it happened on. The information is carried on typed errors, so a program can act on it without matching message strings.
...
- **WHEN** 編譯 `(A B)`
- **THEN** 錯誤指出的偏移量是 3，也就是運算式字串中 `B` 的位置，並附上該處的文字
