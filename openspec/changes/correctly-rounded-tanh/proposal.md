...
- `nn.Tanh` returns the correctly rounded `tanh(x)` for every float32 `x`. This is IEEE 754's recommended correct rounding (§9.2), which CORE-MATH and RLIBM already deliver for binary32, computed the way they compute it:
  ...
  - Between them, a float64 approximation of our own computes `-expm1(-2a)/(2+expm1(-2a))`, with Cody–Waite reduction and a Taylor series. It uses only IEEE 754 `+ − × ÷`, and an explicit conversion rounds every product. Go's specification forbids fusing across such a conversion, so the approximation returns the same bits on every platform. `math.Tanh` does not: it contracts multiply-add on arm64, picks an FMA path at run time on amd64, and is assembly on s390x.
  ...
  - The few inputs inside the margin are listed with their correctly rounded results, the way CORE-MATH lists its exceptional cases. A `math/big` evaluation stays behind the table. It starts at 128 bits and doubles until the result is clear of both midpoints, so an approximation changed without a new table still returns the right answer. `tanh` of a nonzero float32 is transcendental (Lindemann–Weierstrass), so it is never exactly a midpoint and the high-precision path always decides.
