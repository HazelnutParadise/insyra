...
- `cli-failure-reporting`: an error names the command and the argument, and does not leak the standard library's text. (The capability exists on 0.4; on this line it starts with these requirements.)
...
- `Docs/cli-dsl.md` and the CLI skill already describe `read` and `env`'s shapes; the new Forms are generated from the registry at `help` time, and `TestCLIDocsMatchRegistry` still passes because no `Usage` changed.

## Backport to dev (0.3.x)

Dev received all of it: `parseFloatArg`/`parseIntArg` at the thirteen sites and the eight timeseries messages without strconv's wrapped text, case-insensitive `kmeans`/`knn` option keys with the supported list, validated and lowercased `knn` `weighting`/`algorithm`, `varTypeError` for the five commands, the aligned `help` table, `read` and `env` Forms and Examples, the exported `LookupCommand`/`SnapshotRegistry` on this line's `registryMu`, and the nine package-level regular expressions.
