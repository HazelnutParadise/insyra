...
Dev received:
- `ccl.CompileError` and `ccl.EvalError`, re-exported from `engine/ccl`, with byte offsets snapped to a character boundary, rows on evaluation failures, and no Go struct dumps (`ccl-error-reporting`).
- `ErrorInfo.Cause` and `Unwrap`, and the elapsed time dropped from failure messages.
- The follow-up that names the failing statement inside `ExecuteCCL` (`CompiledStatement`, `CompileMultilineStatements`, `AttachExpr`).

Adapted on dev:
- Failures are recorded through `warnErr`, dev's non-sticky Warning-level path with the error kept as the cause, instead of `failErr`.
- The compile-failure example is `(A B)`: dev's parser still accepts `SUM(A B)`, whose comma check stayed on 0.4 with batch 8.
