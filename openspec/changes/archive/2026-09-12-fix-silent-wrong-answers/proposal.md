...
- Dev received all three fixes (`FormatValue` bounded to `int64`, `time.UnixMilli`, `map[int]any` keys through `numberToColIndex`), their tests and both changelog entries. The negative-key test holds on dev as written.
- `date-parsing-coverage` gained the millisecond requirement. `number-text-format` did not exist on dev; it was created holding only the architecture requirement, narrowed to `FormatValue`.
- Stayed on 0.4: `number-text-format`'s output-formatting requirements (their code was not backported); the `delivery-status.md` edit.
