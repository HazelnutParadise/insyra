...
- Dev received all eight panic fixes, their tests, `Docs/gplot.md`, `Docs/plot.md` and the changelog entries.
- Adapted: `CreateHeatmapChart` refuses only a row shorter than row 0; a longer row never panicked and still draws. `CreateBoxPlot` keeps a series whose `Data` was empty to begin with, as before, and drops one only when all its lists were nil. `SavePNG`'s extension check sits in front of dev's default online fallback, which is unchanged.
- Adapted: the `gplot` tests check the written file, because `SaveChart` returns nothing on dev. `error-philosophy` was synced with the constructors named explicitly and both heat-map row cases.
- Stayed on 0.4: the `delivery-status.md` edit.
