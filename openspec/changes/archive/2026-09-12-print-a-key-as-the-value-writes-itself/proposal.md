...
Dev received all of the code: `UncomparableKey`'s unexported `text` field, filled by `ToMapKey` for a `fmt.Stringer`, preferred by `String` under the same truncation limit; the four tests; both changelog entries, and the requirement text, merged into this line's `cell-identity`.

Adapted:
- The changelog names a Parquet `Decimal128` column's `decimal.Decimal` as the example, measured on this line: it is uncomparable and now prints as `decimal.Decimal(-340.0221114815)`. It does not name `finance.ScheduleTable`, which was not checked here.
- The `cell-identity` requirement on this line has no NaN clauses (`nan-is-not-a-map-key` was not backported), so only the display clauses and their two scenarios were added.

Left on 0.4:
- The AGENTS.md follow-up this change removed: this line never had it.
