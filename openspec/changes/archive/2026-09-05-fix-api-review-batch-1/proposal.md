...
Dev received:
- `DataList.ReplaceLast` replacing only the last cell equal to `oldValue` (capability `datalist-value-replacement`).
- `csvxl.AppendCsvToExcel` replacing an existing same-name sheet in full, and the three functions closing their workbooks (capability `csvxl-excel-append`).
