...
- Dev received all fourteen test files, the `engine/ccl.EvalError` doc comment and the `#371` link in `AGENTS.md`. `test-suite-integrity` was synced unchanged.
- Adapted: `gplot`'s save helper checks the written file (`SaveChart` returns nothing on dev), and the quiet helpers drop `SetPanicOnError`. `engine/ccl`'s compile-error case uses `1 +)`, because dev's parser accepts `SUM(A B)`.
- Adapted: `isr`'s `Col(99)`, `Row(99)` and `UseDT(dt{})`/`UseDL(dl{})` assertions check dev's behaviour, a non-nil wrapper holding no table or list.
- Stayed on 0.4: `TestSaveChart_NilChart` and `TestSaveChart_UnsupportedFormat`, which assert `SaveChart`'s error return; the `api-review.md` and `delivery-status.md` edits.
