...
系統 SHALL 回報被選中的那一次旋轉是否收斂，並 SHALL 一路傳到 `FactorAnalysisResult.RotationConverged`。在多個起點之間挑選時，SHALL 優先選擇收斂的解中準則值最小者；若全部都沒有收斂，SHALL 回傳準則值最小者並回報未收斂。系統 SHALL NOT 在缺少收斂資訊時預設回報已收斂。Promax 是封閉式旋轉、沒有迭代，回報已收斂。
