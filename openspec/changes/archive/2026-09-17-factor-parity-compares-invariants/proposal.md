...
Backported. The test code applies unchanged, and the tolerances it introduces measure GPArotation 2026.8.2 and psych, not this line's own rotations. The failing-leaf counts in `stats/factor_analysis_test.go` are `0.4`'s and were not copied; this line's own count is re-measured separately. `oblimin-honours-its-start` and the GPArotation step change stay on `0.4`, so on this line a multi-start oblimin still returns its single-start answer.
