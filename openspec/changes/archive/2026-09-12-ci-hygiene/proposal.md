...
Dev received:
- The `gofmt` formatter in `.golangci.yml`, with the 20 files `gofmt -l .` flagged on dev formatted (comments, blank lines inside doc comments and import alignment only).
- `go vet ./...` on every OS leg, `-race` on the ubuntu leg, coverage in the job summary from the macOS leg, and `fail-fast: false`.
- `permissions: contents: read` on six workflows, `persist-credentials: false` in deploy-docs, and the `AGENTS.md` lint note.
- The windows skip for the two tests that assert POSIX permission bits.

Verified on dev before enabling the checks: `go vet ./...` exited 0, and `go test -race ./...` exited 0 with 37 packages ok and no race report.

Stayed on 0.4:
- The `0.4` push and pull_request triggers: they name a branch that only 0.4's workflows need to cover.
- The formatting hunks for `internal/ccl/batch8_test.go` (not on dev) and for three files already gofmt-clean on dev.
