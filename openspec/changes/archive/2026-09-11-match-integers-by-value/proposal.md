...
Dev received:
- One matcher, chosen once per call, for `Count`, `FindFirst`, `FindLast`, `FindAll`, the `Replace` methods, `DropAll`, `FindRowsIfContains(All)`, `FindColsIfContains(All)`, `DropRowsContain` and `DropColsContain`: integers match by value across Go integer types, a float never matches an integer.
- The CLI's `count`, `find` and `replace` matching integers in CSV-loaded tables and restored variables, through those methods.
- Cells Go cannot compare with `==` no longer panicking these lookups.
- The `Docs/DataList.md`, `Docs/DataTable.md` and CLI usage-reference notes on how a value is matched.

Adapted on dev:
- `FindRowsIfContainsAll` compared with a bare `==` on dev, so a searched `NaN` keeps matching nothing there; every other lookup already matched `NaN` to `NaN`.
- Parity tests run every lookup over floats, `NaN`, strings, bools, `nil` and named non-integer types against the comparisons dev used before, and pass on the code before and after the change.
- Timings measured on dev, a million-cell `Count`: floats 2.3 to 1.7 ms, strings 2.8 to 2.0 ms. `Count(7)` on `int64` cells went from 0.55 ms, when it matched nothing, to 2.4 ms now that it matches.

Stayed on 0.4:
- Encoder categories keyed by integer value (`labelKey`), with its `Docs/DataTable.md` sentence, the `encode … ordinal … order` CLI note and tests: it changes `OneHotEncode`, `LabelEncode` and `OrdinalEncode` results for valid input.
- The `AGENTS.md` `Counter()` follow-up and `delivery-status.md`.
