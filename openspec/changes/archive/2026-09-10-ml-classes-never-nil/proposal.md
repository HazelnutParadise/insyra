...
Dev received:
- `ml/mltest.RunConformance` failing an implementation whose `Classes()` returns nil, instead of crashing on it.
- The redundant `isNilPointer(classes)` check dropped from the two guards in `ml/model_selection.go`.

Stayed on 0.4:
- The ten `Classes()` methods returning an empty list instead of nil, with `noClasses` and the `nn/protocol.go` change: nil becoming non-nil is breaking.
- The `chainable-never-nil` spec, `Docs/ml.md` and skill text: they describe the excluded behaviour.
