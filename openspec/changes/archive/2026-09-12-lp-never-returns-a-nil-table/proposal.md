...
- Dev received the unrelated `gplot` part of the same commit: `CreateBarChart` with no `XAxis` numbers the bars 1..n.
- Dev also received `SolveFromFile` checking for more than one `timeoutSeconds` before `initGLPK`. It still logs a warning and returns `nil, nil`, so an already-wrong call no longer installs GLPK but its return is unchanged.
- Stayed on 0.4: never returning a nil table, `Status: Error` for an unreadable solution file, `Docs/lp.md` and its tests (breaking: a `nil` becomes a table).
- Stayed on 0.4: `SolveModel`'s nil-model and too-many-timeouts checks, since dev has neither and adding one changes a return.
- No requirement was synced: `error-philosophy` on dev does not gain this change's requirements, which describe the 0.4 behaviour.
