...
Dev received:
- Atomic `Config` fields, the bounded and ordered error hook, and the banner moving to the REPL (`core-config-and-error-hook-safety`).
- The `DetectEncoding` sample-boundary fix (`core-encoding-detection`).
- Single-pass `ClearNaNs`/`ClearNils`/`ClearNilsAndNaNs`/`ClearNumbers`/`DropAll`/`ClearStrings`, `Update` naming itself, and `IsEqualTo`/`IsTheSameAs` no longer panicking on uncomparable cells (`datalist-equality-and-cleanup`, trimmed).
- `FindColsIfContains*` without per-column warnings, `strings.Contains`, loop-based `Count`/`Clone`, and sorted new columns in `AppendRowsByColName` (`datatable-find-hygiene`).
- `csvxl` `%w` wrapping and 0755 directories; `parquet.Write` returning the writer's close error and `parquet` logging through Insyra's logger (`io-error-hygiene`, trimmed).
- `asDataList` at every unchecked `*insyra.DataList` assertion in `stats` (`stats-input-type-guard`), including `TwoSampleTTest`, `TwoSampleZTest` and `FTestForVarianceEquality`, which dev still had because batch 2's `stats` part stayed on 0.4.
...
Stayed on 0.4:
- `plot.SavePNG` online fallback becoming opt-in (`plot-png-export-policy`): breaking, changes a default.
- `IsEqualTo`/`IsTheSameAs` treating NaN as equal to NaN: changes a returned value; held for an owner decision.
- `parquet.Write` writing through a temp file and rename: changes how the output file is produced; held for an owner decision.
