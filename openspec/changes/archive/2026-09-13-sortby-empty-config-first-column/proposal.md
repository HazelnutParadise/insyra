...
- Received: a config that names no column sorts by the first column with no error and no warning, documented in `Docs/DataTable.md`; this line never refused it, so for such a config nothing changed.
- Received: the CLI `sort` passes a position as `ColumnNumber`; the `ColumnIndex` conversion from `sortby-column-selection` never reached this line.
- Not on dev: rewriting a BREAKING changelog entry, since this line's entry was written from the net change.
