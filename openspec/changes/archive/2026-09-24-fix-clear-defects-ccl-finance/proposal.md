...
- **`finance`'s `RoundUnnecessary` mode panicked.** It is documented to "panic with decimal.ErrRoundingNecessary", and it does: `NPV(0.03, [0, 1], Options{Scale: 2, Mode: RoundUnnecessary})` takes the program down. On this line `error-philosophy` names the paths that must not end the host program or panic, and a rounding mode the caller chose belongs among them: the point of the mode is to *learn* that rounding was needed.
...
- `finance` gains `Options.finish`, which applies the output scale and, under `RoundUnnecessary`, rounds half-up and compares: a value that changed is reported as an error. All 42 call sites go through it. The other rounding modes are unchanged.

## Capabilities

### New Capabilities

- `ccl-type-semantics`: a literal that cannot be represented, and a format that does not fit, are errors rather than values. (The capability exists on 0.4; on this line it starts with these requirements.)
...
- `internal/ccl/ccl_tokenizer.go`, `internal/ccl/ccl_parser.go`, `internal/ccl/stdlib_typeconv.go`, `finance/options.go` and the 10 finance files that round a result, plus tests.
