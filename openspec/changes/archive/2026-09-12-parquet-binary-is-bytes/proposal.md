...
Dev received the read side: `Binary`, `LargeBinary` and `FixedSizeBinary` read as `[]byte` copied out of Arrow's buffer; `Read` and `recordToDataTable` build a `[]any` column with `Append` through `newColumn`, so a `[]byte` cell is not flattened; the tests for reading, telling a binary column from a text one and rendering the column as hex, plus a JSON test (base64, checked through this line's goccy/go-json path); the docs row, the skill note, both changelogs and the binary requirement of `parquet-column-types`.

Adapted:
- On this line the step before was not a string of raw bytes: `parquet-foreign-column-types` left binary columns reading as the text of the whole array. The changelog describes the net change from v0.3.2 (that text in every row, now `[]byte`) and does not mention a string intermediate.
- A test pins that `Write` still writes a column of `[]byte` cells as a string column, and the docs, skill and AGENTS.md round-trip follow-up say so.

Left on 0.4:
- `inferArrowType` and `appendValue` writing a column of `[]byte` cells as Arrow `Binary`, with `TestABinaryColumnSurvivesARoundTrip` and the "A binary column survives a round trip" requirement: breaking, it changes what `Write` puts in a file for a table holding `[]byte` cells, which this line writes as a string column.
- The changelog's **BREAKING** mark and its `.(string)` migration note: they describe 0.4's string intermediate.
