...
- Dev received all of it: `FilterWithCCL` keeping every batch, the error-channel read in `FilterWithCCL`, `ApplyCCL` and `Stream`, the `os.ErrClosed` guards, the tests, `Docs/parquet.md` and both changelog entries. Dev's `ApplyCCL` also writes through its own temporary file, so the "overwrite the original" wording holds there.
- Stayed on 0.4: the `delivery-status.md` edit (0.4 ledger).
