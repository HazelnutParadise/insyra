...
- `SanitizeHistoryLine` masks a `password=`/`pwd=` value to its closing quote, the next `;` or the next ` key=`, not to the first space (backport review): `db connect pg "host=h password=a b"` was recorded as `password=*** b"`, leaving part of the password in history.
