...
Adapted on dev:
- The REPL saves every non-empty line to history, comments, `exit` and lines without tokens included, before deciding what the line does (backport review). With automatic saving turned off for masking, the hand-written save ran after those checks, so they no longer reached history.txt as readline had saved them.
- `SanitizeHistoryLine` masks a `password=`/`pwd=` value to its closing quote, the next `;` or the next ` key=`, not to the first space (backport review): `db connect pg "host=h password=a b"` was recorded as `password=*** b"`, leaving part of the password in history. The key also accepts spaces around `=`, a backslash escapes the next character inside single quotes, and `}}` stands for `}` inside braces (second backport review): `password = s3cr3tA` was left unmasked, and `password='it\'s s3cr3tB'` and `PWD={ab}}s3cr3tC}` kept the rest of the password.
- A row-independent aggregate argument is passed as a whole column only when the argument is a sequence-function call (backport review). Deciding by whether its length equalled the row count made `ZZLEN(@.0)` 3 on a 3x3 table and 1 on a 2x3 one; v0.3.2 gave 1 for both.
- The shift, window and `REPEAT` guards refuse only what differs by platform or panics (backport review): `NaN`, infinities, values outside int64, a repeat count of -1 or less and a `REPEAT` result whose length overflows `int`. A count between -1 and 0 truncates to 0 and gives `""`, as `int(n)` did on v0.3.2 (second backport review: refusing every negative count made `REPEAT('ab', 0-0.5)` an error). The int32 bound and the 64 MiB `REPEAT` cap refused arguments v0.3.2 handled: `LAG(A, 3000000000)` and `ROLLING_MEAN(A, 3000000000)` gave nil cells, `LEN(REPEAT('', 100000000))` gave 0 and `REPEAT('ab', 40000000)` an 80 MB string. A shift is clamped to the column length and the rolling buffer to the row count, so those values neither overflow nor over-allocate.
