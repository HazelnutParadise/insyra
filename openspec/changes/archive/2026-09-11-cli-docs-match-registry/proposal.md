...
Dev received:
- The `pca`, `regression` and `count` Usage strings and usage errors, and the `save … sql` usage error listing `rownames [true|false]`.
- `TestCLIDocsMatchRegistry`, `TestCLITopicListsNameEveryCommand` and `TestUsageNamesWhatTheCommandParses`.
- `Docs/cli-dsl.md` and the three `use-insyra-cli` references matched to dev's registry, with `accel` documented and the replaced examples re-run on dev in an isolated home directory.

Adapted on dev:
- `accel`'s documented usage has no `--precision` and its notes do not mention the flag, because dev's Usage does not list it; `plot` keeps `[options...]`, which dev's Usage still shows.

Stayed on 0.4:
- The changelog correction of batch 7's `--precision` entry: that entry is not on dev.
- The `AGENTS.md` follow-ups (CLI integer matching, ignored arguments): 0.4 later replaced them, and the integer one is resolved by `match-integers-by-value`.
