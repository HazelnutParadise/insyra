...
Dev received:
- `GroupBy` copying the column data it groups, so `Aggregate` reads the snapshot and never the parent (`groupby_snapshot_test.go`, including the `-race` case).
- `AtomicDoAll` skipping a nil `*DataList`, a nil `*DataTable` or a nil value instead of panicking, with its signature kept as `...any`.
- The `Docs/CCL.md` custom-function section and its locking rule, re-run on dev.
- `stats`' nil-table test restoring the log level it changes.

Stayed on 0.4:
- `AtomicDoAll(...Lockable)`, `Lockable` and `lockHandle`: code passing `any` would stop compiling.
- `ExecuteCCL` all or nothing: a failed script would leave the table unchanged instead of partly applied, a changed result.
- `TestMain` and the extended `restoreConfig`: dev's root tests have no `init()` to replace and already restore what they change; `mkt` already restores through `quietMktLogs`, and the `ml` test file exists only on 0.4.
