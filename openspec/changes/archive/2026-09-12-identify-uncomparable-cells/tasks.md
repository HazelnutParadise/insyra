...
- [x] 3.2 `ToMapKey(v any) any`，用 `reflect.ValueOf(v).Comparable()`，純量先走 type switch 快路。
- [x] 3.3 `DataList.Counter` 與 `DataTable.Counter` 改用 `ToMapKey`。
...
- [x] 5.7 `counter[ToMapKey(v)]` 查得到；`counter[1]`、`counter["a"]` 照舊。
- [x] 5.8 列印含大型二進位值的計數結果仍可讀。

## 6. 收尾
- [x] 6.1 `gofmt -l .`、`go build ./...`、`go test ./...`、`golangci-lint run`。
- [x] 6.2 兩份 CHANGELOG。
- [x] 6.3 `Docs/DataList.md`、`Docs/DataTable.md` 補 `ToMapKey` 與 `UncomparableKey`；skill 若提到 Counter 一併更新。
