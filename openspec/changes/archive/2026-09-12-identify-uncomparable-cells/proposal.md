...
- Within one call, `writeCellValue` reuses the encoding of a slice or map that holds another slice or map and was already written at the same depth, and writes such a nested encoding longer than 1,024 bytes as its SHA-256 digest (second backport review). A value sharing sub-values without containing itself, `x = []any{x, x}` repeated, has 2^depth paths: `Count` took 0.68 s at depth 22 and never finished at depth 30. The digest depends only on content, so equal values match whether or not their sub-values are shared. A container that refers back past itself is not reused, because its back-reference distance holds only where it was written.
