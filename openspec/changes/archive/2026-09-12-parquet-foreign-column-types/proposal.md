...
- `Mean`, `Sum` and the rest of the numeric path do not read a decimal column. Measured afterwards and corrected here: they do not refuse it either. On the 0.3.x line they skip each cell they cannot read, recording a warning on `Err()`, and an all-decimal list gives `NaN`. Making decimals numeric library-wide is a separate decision and is recorded as a follow-up rather than taken here.
