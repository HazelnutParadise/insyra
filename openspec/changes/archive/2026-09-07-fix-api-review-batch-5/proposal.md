...
The code reached dev in 181dcb0 ("fix: backport the review's non-breaking correctness fixes to the 0.3.x line"), together with its changelog entries; this archive and its specs follow it.

Dev received:
- Exact integer comparison in `CompareAny`, so `Sort`, `SortBy` and `Pivot` order large integers correctly (`exact-numeric-ordering`, trimmed).
- The corrected `HermiteInterpolation` basis (`interpolation-correctness`).
- Distinct initial centres in `KMeans` (`clustering-initial-centers`).
- The zone-less `TryParseTime` layouts (`date-parsing-coverage`).
- `ShowTypes` column order and the `ShowRange` documentation (`display-ordering`).
- `Close()` no longer discarding a queued operation (`actor-close-semantics`).
