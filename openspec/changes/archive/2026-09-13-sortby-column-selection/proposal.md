...
Dev received this change's net result together with `sortby-empty-config-first-column`, in one commit.

- Received: every level is resolved before any row moves; a missing index, name or number records the error on `SortBy` (through this line's `warn`, which logs and sets `Err()`) and leaves the table unchanged; several selectors sort by precedence with a warning.
- Adapted: the several-selectors warning is logged with `LogWarning` only, because this line's `warn` also sets `Err()` and would turn a sort that ran into a reported failure.
- Adapted: on this line the missing-column errors used to come from `GetCol`, `GetColByName` or `GetColByNumber`; an out-of-range `ColumnIndex` did set `Err()` here, so the table row saying it stayed nil is 0.4's measurement.
- Adapted: the CLI `sort` sets only the field its argument resolves to, instead of `ColumnNumber: -1` next to a name, and passes a position as `ColumnNumber`; its output and exit status are unchanged.
- Not on dev: refusing a config that names no column and the `ColumnIndex: "A"` migration, reversed on 0.4 before release by `sortby-empty-config-first-column`.
