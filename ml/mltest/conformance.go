...
//
// A nil *insyra.DataList panics on every method it has, Err() included.
// ml.Classifier is a public interface with only exported methods, so an
// implementation outside this repository can return nil from a fitted model,
// and this suite is where it finds out — rather than crashing on the Len()
// that follows.
func checkClassesUsable(classes *insyra.DataList) error {
	if classes == nil {
		return fmt.Errorf("Classes() returned nil for a fitted classifier; it must return the classes the model was fitted on")
	}
	return nil
}
