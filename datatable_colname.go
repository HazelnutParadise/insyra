...
// number, or "" when the column has no name. A negative index counts back from
// the end. Out of range logs a warning, sets Err() and returns "".
...
// name. When there is none it logs a warning, sets Err() and returns -1.
