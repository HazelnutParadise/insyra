...
func TestRealModelSmoke(t *testing.T) {
	path := os.Getenv("INSYRA_NN_REAL_MODEL")
	if path == "" {
		t.Skip("INSYRA_NN_REAL_MODEL is not set")
	}
	...
	for _, spec := range model.Outputs() {
		...
		fmt.Printf("INSYRA_NN_REAL_MODEL output %q shape %v\n", spec.Name, output.Shape())
	}
}
...
func requireONNXReference(t *testing.T) string {
	...
	if err != nil {
		reftest.Missing(t, "python3", "the nn ONNX parity checks", err)
		return ""
	}
	...
	if err := command.Run(); err != nil {
		reftest.MissingOutput(t, "python3 with numpy, onnx, and onnxruntime", "the nn ONNX parity checks", err, stderr.Bytes())
		return ""
	}
	return python
}
