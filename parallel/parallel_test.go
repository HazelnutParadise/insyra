...
func TestParallelGroup_NoResults(t *testing.T) {
	// Functions that do not return values. atomic.Int64 because the two
	// closures execute on different goroutines.
	var counter atomic.Int64
	fn1 := func() { time.Sleep(10 * time.Millisecond); counter.Add(1) }
	fn2 := func() { time.Sleep(10 * time.Millisecond); counter.Add(1) }

	pg := parallel.GroupUp(fn1, fn2)
	pg.Run().AwaitNoResult()

	if got := counter.Load(); got != 2 {
		t.Errorf("Expected counter to be 2, got %d", got)
	}
}

func TestParallelGroup_Mixed(t *testing.T) {
	// Mixed functions: one with result, one without. Only fn2 mutates the
	// counter, but it still runs on a separate goroutine, so use atomic
	// for visibility / vet-cleanness.
	var counter atomic.Int64
...
func TestParallelGroup_NoResults_WithAwaitResult(t *testing.T) {
	...
	// `counter` is touched concurrently by fn1 and fn2 — use atomic to
	// avoid the data race that the race detector flagged.
	var counter atomic.Int64
	fn1 := func() { time.Sleep(10 * time.Millisecond); counter.Add(1) }
	fn2 := func() { time.Sleep(10 * time.Millisecond); counter.Add(1) }
