...
**What a device result is.** A float32 result that may come from a device is the correctly rounded value of the exact operation: IEEE 754's recommendation for elementary functions (§9.2), applied to sums as well. `EdgeSum` is the exact sum of its products rounded once; `tanh(x)` is the true value rounded once. Two implementations that each deliver that value agree whatever their evaluation order, core count or hardware, so neither is defined by imitating the other. A result that is exactly zero is `+0`, as when accumulating into a zeroed output.

**How a device kernel computes it.** In integers. WGSL guarantees correct rounding for `+ - *` but lets an implementation flush subnormals to zero, ignore the sign of zero, reassociate and contract, and bounds `/`, `exp`, `sqrt` and `tanh` only loosely; its integer operations are exact. A kernel therefore holds float32 values as bit patterns (`array<u32>`, never materialised as `f32`), forms exact products and sums in integer limbs (a Kulisch long accumulator, as ExBLAS does on GPUs), and rounds once to nearest-even in integer code. An exact sum does not depend on the order it is added in, so a kernel may split one sum across threads freely.

**The device register.** [`accel/internal/wgpu/exact_sum.go`](accel/internal/wgpu/exact_sum.go) counts in units of 2^-298, like `nn`'s CPU accumulator. It holds 27 digits of 24 bits in `u32` and defers carries for up to 64 additions. A product always spans three digits, and the deferral bound keeps every digit's true value below 2^31, so nothing ever wraps. Every operation is unsigned, so no result depends on how a shader compiler treats signed overflow or the right shift of a negative number. The library also uses no `select()`: gogpu/naga v0.19 writes a scalar `select()` that is an operand of another expression as an unparenthesized ternary in Metal. That silently changed a carry until the device parity tests caught it; it is reported upstream as gogpu/naga#95. A kernel that needs more exact arithmetic extends this library instead of writing its own.

**How that is verified.** By construction, and exhaustively wherever the input space allows: a unary float32 function is compared on all 2^32 inputs against a high-precision oracle. Sums are compared against `math/big` on random inputs and on the inputs that break careless implementations: cancellation, subnormals, overflow, NaN, infinities and signed zeros. An exhaustive run on one machine proves a function only for platforms that compute the same bits, so a CPU fast path behind correct rounding uses only IEEE 754 `+ − × ÷` with an explicit conversion around every product that feeds an addition or subtraction, and no `math` function, whose implementation differs by platform. Go's specification forbids fusing a product across such a conversion, and gc performs no other floating-point fusion. A CI test hashes the fast path's bits over its whole input range, so every platform CI runs confirms them. `nn.Tanh` is the pattern: a deterministic float64 approximation (`tanhFloat64`), a margin above its exhaustively measured error, and a table of the few inputs inside the margin, with `math/big` behind the table.
...
- **Device MatMul's bit-parity is platform-dependent, not kernel-dependent.** It holds because Metal and Go/arm64 contract multiply-add identically, which WGSL does not promise: it permits contraction, reassociation and flush-to-zero. MatMul predates the integer rule above, so its parity is measured where it runs and cannot be inferred for a platform nobody has run on — risk: the device path is verified on Apple and Metal only, and a conforming implementation that reassociates would break it.
- **The pure-Go GPU stack is one maintainer.** gogpu/wgpu, naga and goffi are substantially one person's work, 159 stars, and CI exercises no GPU hardware. Its translation is not always right, either: naga v0.19 miscompiles a `select()` used as an operand (see the device register above) — risk: the observable-fallback design is the mitigation, device results are compared bit for bit rather than trusted, and no roadmap item should assume the dependency survives.
