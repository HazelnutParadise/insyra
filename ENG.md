...
- **`Predict` returns a `DataList` whatever the model is.** Nothing in the type distinguishes a predicted quantity from a cluster id or a class — risk: this produced four separate defects where an interface was satisfied in form but not in substance, and the mitigation is that a model must *declare* what it predicts (`Classifier`, `Clusterer`), not that a check can infer it.
