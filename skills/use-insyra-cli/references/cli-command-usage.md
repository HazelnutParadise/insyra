...
When a command looks for a value (`count`, `find`, `replace`), an integer matches integer cells of any Go type, so `count x 3` finds the 3s in a CSV-loaded column, which stores them as `int64`. It does not match `3.0`: type `3.0` to find a float.
