...
Every registered command appears here, grouped by topic, and a test in `cli/commands` fails when one is missing. For a command's exact usage, run `insyra help <command>`.
