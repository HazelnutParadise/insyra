...
- **Variables persist per environment.** An environment keeps its variables in `~/.insyra/envs/<name>/state.json` and its history in `history.txt`. Every one-shot command restores the environment first, so `insyra newdl 1 2 3 as x` followed by a separate `insyra mean x` works. The environment is `default` unless you pass `--env <name>` or run `env open <name>`.
...
- **Scripts keep going after an error.** An `.isr` file holds one command per line, with `#` comments, quoting and backslash escapes. A failing line is reported and the next line runs, so read the output instead of assuming the script succeeded.
- **The Go DSL is the same language.** `engine/dsl` runs the same lines inside a program and saves state after each successful command. Use it when a Go program should hand the user a scriptable surface.
...
2. **Isolate the work.** Use an environment of its own for each task, so you neither read nor overwrite someone's variables in `default`. `env export` hands the state over. Importing into an environment that already has variables needs `--force`, and that replaces what was there; treat it as destructive and confirm first.
...
7. **Mind the shell.** PowerShell expands `$result`, so quote it as `"$result"`. Quote any token that contains spaces.
