...
When a number has to stay exact — money, rates, anything where `0.1 + 0.2` must be `0.3` — use [`github.com/TimLai666/go-decimal`](https://github.com/TimLai666/go-decimal). It is the decimal type Insyra is built around: every [`finance`](/Docs/finance.md) function takes and returns it, a Parquet `Decimal128` column reads as it, and a column of them works with `Mean`, `Sum`, sorting and the rest like any other number column.
