	...
	if config.UseCalendar {
		...
		for _, p := range points {
			...
			case time.Time:
				// ok
			default:
				insyra.LogWarning("plot", "CreateHeatMap", "calendar mode requires X axis values to be time.Time, got %T", p.X)
				return nil
			}
		}
		if config.CalendarOpts == nil {
			insyra.LogWarning("plot", "CreateHeatMap", "calendar mode requires CalendarOpts to be set")
			return nil
		}
