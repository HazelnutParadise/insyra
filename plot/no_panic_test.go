...
// A series whose Data is empty, rather than full of nil lists, never panicked
// and still gives a chart, as it always did.
func TestCreateBoxPlot_EmptySeriesStillGivesAChart(t *testing.T) {
	quiet(t)

	for name, data := range map[string][]insyra.IDataList{
		"empty slice": {},
		"nil slice":   nil,
	} {
		t.Run(name, func(t *testing.T) {
			c := CreateBoxPlot(BoxPlotConfig{Title: "t"}, BoxPlotSeries{Name: "s", Data: data})
			if c == nil {
				t.Fatal("a series with no lists no longer gives a chart")
			}
			renderToHTML(t, c)
		})
	}
