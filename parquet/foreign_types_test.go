...
// Err() on this line holds the last error recorded, so with more than one
// unreadable column it names the last of them; each one is still logged.
func TestReadSeveralUnrepresentableColumnsNamesTheLast(t *testing.T) {
	fields := []arrow.Field{
		{Name: "clock", Type: arrow.FixedWidthTypes.Time32s, Nullable: true},
		{Name: "ok", Type: arrow.PrimitiveTypes.Int64, Nullable: true},
		{Name: "span", Type: arrow.FixedWidthTypes.Time64us, Nullable: true},
	}
	path := writeForeignParquet(t, fields, func(b *array.RecordBuilder) {
		b.Field(0).(*array.Time32Builder).AppendValues([]arrow.Time32{1, 2}, nil)
		b.Field(1).(*array.Int64Builder).AppendValues([]int64{1, 2}, nil)
		b.Field(2).(*array.Time64Builder).AppendValues([]arrow.Time64{3, 4}, nil)
	})
	...
	e := dt.Err()
	if e == nil {
		t.Fatal("unreadable columns were not reported")
	}
	if !strings.Contains(e.Message, `"span"`) || strings.Contains(e.Message, `"clock"`) {
		t.Errorf("Err() = %q, want the last unreadable column, span", e.Message)
	}
	for row := 0; row < 2; row++ {
		if got := dt.GetElementByNumberIndex(row, 1); got != int64(row+1) {
			t.Errorf("readable column row %d: got %v (%T)", row, got, got)
		}
	}
}
...
func TestSupportedTypesAreExactlyWhatGetValHandles(t *testing.T) {
	...
	for _, dt := range types {
		...
		// getVal's arms are chosen by the array's Go type, not by the value,
		// so a one-null array is enough to ask which arm it would take.
