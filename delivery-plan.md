...
Four `stats` changes, all proposed and none implemented: `add-stats-regression-predict`, `add-kmeans-assign`, `expose-glm-link`, `add-pca-transform-parameters`. They are independent of one another, and the first blocks the most — without a prediction path there is nothing for `insyra/ml` to wrap. `add-kmeans-assign` is the one that also matters to acceleration: assigning a row to the nearest centre is the only selection-shaped operation `stats` will have, and therefore the only one a device may accelerate by default.
