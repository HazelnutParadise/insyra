...
func TestDT_RowSelectors(t *testing.T) {
	...
	// As with Col, an index that is not there finds no row on the 0.3.x line.
...
// A dt or dl value whose embedded pointer is nil comes back as a non-nil
// wrapper. On the 0.3.x line it still wraps that nil pointer; nothing is
// recorded on it.
func TestUse_ValueWithNilPointer(t *testing.T) {
	...
	if table.DataTable != nil {
		t.Error("UseDT(dt{}) invented a table")
	}
	...
	if list.DataList != nil {
		t.Error("UseDL(dl{}) invented a list")
	}
}
