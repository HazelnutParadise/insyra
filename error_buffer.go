...
// warning and fatal insyra records lands here so a program can inspect what
// happened. It is NOT the error-handling API. Handle errors through the error a
// function returns, or through Err()/PopErr() on the instance you called: the
// global buffer mixes records from every goroutine and every object, so what
// you pop may belong to someone else.
