...
func (dt *DataTable) Summary() {
	...
	fmt.Println(utils.ColorText("1;36", tableTitle))
	fmt.Println(strings.Repeat("=", min(width, 80)))

	// Check if DataTable is empty
	if len(columns) == 0 {
		fmt.Println(utils.ColorText("2;37", "Empty dataset"))
		return
	} // Display overall table information
	fmt.Printf("Dimensions: %s rows × %s columns\n",
		utils.ColorText("1;36", fmt.Sprintf("%d", rowCount)),
		utils.ColorText("1;36", fmt.Sprintf("%d", len(columns))))
	...
	fmt.Println("\n" + utils.ColorText("1;36", "Table-wide Statistics"))
	fmt.Println(strings.Repeat("-", min(width, 50)))

	fmt.Printf("Total elements: %s\n",
		utils.ColorText("1;36", fmt.Sprintf("%d", totalElements))) // 顯示各類型資料的比例
	if totalElements > 0 {
		fmt.Printf("Data types: %s numeric (%.1f%%), %s string (%.1f%%), %s boolean (%.1f%%), %s other (%.1f%%)\n",
			utils.ColorText("1;36", fmt.Sprintf("%d", numericCount)),
			float64(numericCount)/float64(totalElements)*100,
			utils.ColorText("1;36", fmt.Sprintf("%d", stringCount)),
			float64(stringCount)/float64(totalElements)*100,
			utils.ColorText("1;36", fmt.Sprintf("%d", boolCount)),
			float64(boolCount)/float64(totalElements)*100,
			utils.ColorText("1;36", fmt.Sprintf("%d", otherCount)),
	...
	if len(numericValues) > 0 {
		...
		fmt.Println("\n" + utils.ColorText("1;36", "Numeric Data Statistics (Across All Columns)"))
		fmt.Println(topLine)
		fmt.Printf(utils.ColorText("1;32", headerFmt), "Statistic", "Value")
	...
	fmt.Println("\n" + utils.ColorText("1;36", "Column Overview"))
	fmt.Println(strings.Repeat("-", min(width, 50)))
...
func displayColumnOverviewTable(columns []*DataList, width int) {
	...
	// 打印表頭
	fmt.Println(topLine)
	fmt.Printf(utils.ColorText("1;32", headerFmt), "Column Name", "Data Type", "Quick Statistics")
	fmt.Println(dividerLine)
