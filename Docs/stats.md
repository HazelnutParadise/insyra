...
| Factor analysis | `psych::fa`, `psych::principal`, `psych::smc`, `psych::KMO`, `psych::cortest.bartlett`; rotations through GPArotation | — | — | 2e-5 per element. Factor frames are compared up to order and sign, a gradient-projection rotation by whether the loadings say it is the same solution (1e-2), with the rotation-invariant L·Φ·L', S·L' and W·L' held to 2e-5 and 1e-4. Opt-in with `INSYRA_STRICT_FACTOR_R_PARITY=1`; the known remainder is itemised at `factorParityTol` in `factor_analysis_test.go` |
