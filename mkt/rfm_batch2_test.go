...
// A numeric string amount has always been read as its number (conv.ParseF64
// parses strings); skipping unreadable amounts must not change that.
func TestRFMReadsNumericStringAmounts(t *testing.T) {
	quietMktLogs(t)
	table := func(amounts ...any) insyra.IDataTable {
		return insyra.NewDataTable(
			insyra.NewDataList("c1", "c1", "c2", "c3").SetName("id"),
			insyra.NewDataList("2024-01-01", "2024-02-01", "2024-03-01", "2024-03-05").SetName("day"),
			insyra.NewDataList(amounts...).SetName("amt"),
		)
	}
	cfg := RFMConfig{CustomerIDColName: "id", TradingDayColName: "day", AmountColName: "amt", NumGroups: 2}
	want := RFM(table(10, 20.5, 30, 5), cfg)
	got := RFM(table("10", " 20.5 ", "30", 5), cfg)
	if want == nil || got == nil {
		t.Fatalf("RFM returned nil: want=%v got=%v", want, got)
	}
	if !reflect.DeepEqual(got.Data(true), want.Data(true)) {
		t.Fatalf("string amounts read differently:\n got  %v\n want %v", got.Data(true), want.Data(true))
	}
}
