...
func (dt *DataTable) sortColumnPosition(level int, config DataTableSortConfig) (int, string) {
	...
	if len(given) > 1 {
		// Logged only: the sort still runs, and dt.warn would also set Err(),
		// turning a successful sort into a reported failure.
		LogWarning("DataTable", "SortBy", "level %d gives %s; sorting by %s and ignoring %s",
	...
	case "ColumnIndex":
		...
		// The same name fallback GetCol uses, so the two agree.
