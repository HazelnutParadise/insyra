...
### [2026-07-17] — CCL compile-time recursion has no depth bound: adversarial input crashes the whole process (fatal stack overflow)
- **Where**: `internal/ccl/ccl_parser.go:197` (parseExpression↔parsePrimary mutual recursion), `internal/ccl/ccl_compiler.go:88` (`Bind`), `internal/ccl/ccl_evaluator.go:122` (`IsRowDependent`, and sibling `containsRowIndex`)
- **What**: Discovered while verifying issue #184. `maxEvalDepth` bounds only the *evaluator*; nothing bounds recursion at tokenize/parse/compile time. Measured on go1.25.12 windows/amd64 (1GB default stack): `strings.Repeat("(", 1_300_000)` crashes in the parser; `"1" + strings.Repeat("+1", 4_000_000)` parses fine (iterative precedence loop) but crashes in `Bind`, which runs in `applyCCLOnDataTable` before evaluation. Go stack overflow is a **fatal error, not a recoverable panic** — the `recover()` in the CCL entry points cannot catch it, so one adversarial formula string kills the entire host process. Reachable from every public CCL entry (`AddColUsingCCL`, `EditCol*UsingCCL`, `ExecuteCCL`).
- **Mitigating factor**: needs MB-scale input (~1.2MB+), so capping formula length at the application boundary is an effective stopgap for services exposing CCL to end users.
- **Suggestion**: add a compile-time bound — either cap input length/token count at `CompileExpression`/`compileStatement` entry, or thread a depth counter through `parsePrimary`, `Bind`, `IsRowDependent`, `containsRowIndex` (mirroring `maxEvalDepth`) so over-deep input returns an error. Behavior change for extreme-but-valid formulas, so the cap value needs a decision; drive via OpenSpec.
- **Status**: pending
