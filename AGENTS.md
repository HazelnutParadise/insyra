...
- **How a function takes settings.** Ruled by the owner on 2026-09-25 while deciding #213:
  - **Four or more settings** go in an options struct, taken as an optional trailing `opts ...XxxOptions` so the plain call stays short (`dt.Describe()`). More than one struct is an error — never "first wins" or "last wins".
  - **Three or fewer** are ordinary parameters. Only the last one, and only when it has an obvious default, may be left out, as a trailing `...T`; more than one value is an error reported through the function's normal error path, never ignored (`dl.Sort()`, `dl.Sort(false)`, while `dl.Sort(true, false)` fails).
