...
- **No option functions.** Insyra does not use the `WithXxx(...)` option-function pattern (`Describe(WithPercentiles(0.1, 0.9))`), and never has. A setting a caller can change is an ordinary parameter or a field of an options struct, because both appear in the function's own signature and documentation; option functions are scattered across the package, so someone who does not already know the library cannot see what can be set. The owner made this a rule on 2026-09-25 while deciding #213.
