...
### [2026-09-12] — the strict factor parity suite is 5,300 sub-tests red against current R, and its own numbers say ~595
- **Where**: `stats/factor_analysis_test.go` (`factorParityTol`, `requireFactorAnalysisRTools`, `assertFactorAnalysisMatchesR`), `stats/crosslang_helpers_test.go` (the baseline cache)
- **What**: run on 2026-09-12 with baselines regenerated under psych 2.6.5 and GPArotation 2026.8.2, `INSYRA_STRICT_FACTOR_R_PARITY=1` fails 5,589 leaf sub-tests before `oblimin-honours-its-start` and 5,334 after; the comment at `factorParityTol` says ~595 on three adversarial datasets. R moved: GPArotation rewrote its engine in 2026.4-1 (default `algorithm = "bb"`, `maxit = 2000`), psych now runs 20 unseeded random starts and picks by hyperplane count. The suite then measures the wrong things: `rotation_matrix` is compared element by element, so a column swap or a sign flip counts as a difference of 2.0 (1,311 of the leaves); `simplimax`, `bentlerQ`, `geomin` and `promax` land in different local minima from R on a third of the combinations, at either default. The cache under `testdata/baseline_cache/` is keyed on script and payload only, never on the package versions, so the 2026-08-01 baselines answered for whatever psych produced them until moved aside. Measured with a sign-and-permutation-invariant loadings distance instead, MINRES over 16 datasets × 10 rotations: 112 of 160 within 1e-4 of psych at `Restarts: 1`, 119 at the new default of 20; oblimin, the default, is 13 of 16 at both, the other three being extraction-level drift of 1e-4 to 3e-3 on 10-row tables. Where the two disagree on a multimodal criterion, the criterion value decides: on `simplimax` our 20-start solution is below psych's on two datasets (0.1445 vs 0.1599, 0.1657 vs 0.2270) and equal on the four where `Restarts: 1` had been stuck above it.
