...
| M35 | Device arithmetic is exact | planning | pending | a WGSL library that decodes float32 bit patterns, forms exact products, accumulates them in integer limbs and rounds once to nearest-even returns the CPU's bits on adversarial and random inputs on the M3 |
| M36 | `EdgeSum` runs on the device | planning | pending | production kernels for `EdgeSum` and both gradients, built on M35, equal to the CPU bit for bit and wired into `nn` behind a size floor measured on a ladder, with observable fallback |
| M37 | `Tanh` runs on the device | planning | pending | the device `tanh`, in integer arithmetic, equals `nn.Tanh` on all 2^32 inputs |
| M38 | Tensors stay on the device | planning | pending | `nn` tensors carry the device they live on; `To` and `ToHost` move them explicitly; operations with device kernels keep their results there, others run on the host with a report, strict mode refuses; one tape differentiates across devices; a recurrent step's speed on the device is measured against the all-core CPU |
...
`device-exact-product-sum` (M35): a WGSL library that sums float32 products exactly in unsigned integers returns `nn.EdgeSum`'s bits on random and adversarial rows on the M3, and its throughput against the all-core CPU is recorded.

## Next Ticket
`device-exact-product-sum` (M35): the device's exact arithmetic, built and verified on its own before any kernel uses it. Then M36 through M39 in order; M33 and M34 are done.
...
- decision: `nn.Tanh` is correctly rounded on every platform (M34), computed the way CORE-MATH and RLIBM compute binary32 functions. `|x| < 2^-13` and `|x| >= 9.5` are settled by proof. Between them a float64 approximation of our own, `tanhFloat64`, uses only IEEE 754 `+ − × ÷`, and an explicit conversion rounds every product that feeds an addition. So it returns the same bits on every platform, which a CI test confirms with a hash over every input between the analytic ranges. Its float32 rounding is accepted when it lies more than 16 float64 steps from every float32 midpoint. The four inputs inside that margin are a table, and `math/big` stays behind the table. `Tape.Tanh`'s gradient rounds every step and fuses none.
  rationale: `math.Tanh` was not an option for the fast path. It fuses multiply-adds on arm64, picks an FMA path at run time on amd64 (`math.Exp`) and is assembly on s390x, so an exhaustive run on one machine would prove nothing about another. The adversarial review of the first version caught this. `tanhFloat64` is deterministic, so one exhaustive run bounds it everywhere. On darwin/arm64, over every float32 input, its largest error is 2.63 float64 steps. The nearest any input comes to a midpoint is 5 steps, 32 inputs come within 64 steps, and 4 come within the margin of 16. All 2^32 inputs were checked, 0 mismatches. The 271,581,184 between the analytic ranges were compared against an independent `math/big` oracle, itself anchored to 60-digit values from Python's `decimal`. The rest were compared against their proven values, which `TestTanhAnalyticRangesAgreeWithOracle` checks against the oracle on samples. The old `float32(math.Tanh(float64(x)))` was also correct on every input here, so no forward result changes on arm64. Cost, one core, 2^20 inputs: normal(0, 2) activations 8.3 ns before, 7.5 ns now; random bit patterns 7.6 ns before, 5.5 ns now. The gradient changes 24,892 of 100,000 random pairs on arm64, which fused `1 - y*y`. amd64 does not fuse that float32 expression at `GOAMD64` v1 or v3 (checked in the compiled assembly), so it already produced these bits.
  timestamp: 2026-09-26
  impacted_ticket_ids: correctly-rounded-tanh (M34); M37 takes this as its reference

- decision: `EdgeSum` and both gradients are the correctly rounded exact sums of their products (M33), computed by a float64 fast path that proves the rounding when it can and an exact 640-bit integer register when it cannot. The CPU cost of exactness is recorded, not avoided.
  rationale: measured on the 8-core M3 with `BenchmarkEdgeSum` on every core, best of 5 runs of 5 iterations, each version compiled once and run after the load settled. Against the fixed-order version it replaced, the exact register alone was 3.1–23.6× slower (10k nodes, 100 edges each, B=1, reverse: 0.98 → 23.0 ms). Adding the products in float64, where each is exact, and accepting float32(S) only when Higham's bound for the sum (§4.2) keeps [S−B, S+B] clear of every float32 midpoint (Ziv's method) brings it to 1.05–2.28×: 1.35× forward at 10k nodes, 10 edges, B=1 (0.09 → 0.12 ms); 1.10× forward at 100k nodes, 100 edges, B=16 (488 → 538 ms); 1.79× reverse at 1M nodes, 10 edges, B=16 (1,143 → 2,041 ms). On random data with a quarter of the sums built to cancel, the fast path settled 75% of sums and the register the rest. What the price buys: the result no longer depends on edge order, batch order, core count or platform, it is the most accurate float32 there is, and a device can reproduce it in integer arithmetic under WGSL's guarantees.
  timestamp: 2026-09-25
  impacted_ticket_ids: exact-edge-sum (M33)

- decision: A float32 result that may come from a device is defined as the correctly rounded value of the exact operation, and device kernels compute it in integer arithmetic. `EdgeSum` and its gradients become the exact sum of their products rounded once (replacing the fixed ascending order), `nn.Tanh` becomes correctly rounded, and #379 proceeds to production device kernels (M36) and device-resident tensors (M38), the owner having ruled for both on 2026-09-25 and delegated the technical design.
  rationale: WGSL guarantees correct rounding only for `+ - *` and still lets an implementation flush subnormals, ignore the sign of zero, reassociate and contract; `/`, `exp`, `sqrt` and `tanh` are bounded loosely. The xor that kept Metal from fusing (`measure-edge-sum-device`) therefore worked by observation, not by any guarantee. Integer operations are exact under the spec. Defining results as correctly rounded (IEEE 754 §9.2's recommendation for elementary functions; the approach of CORE-MATH and RLIBM for binary32) makes the CPU and the device agree by definition rather than by one imitating the other. An exact sum rounded once (the Kulisch long accumulator; ReproBLAS and ExBLAS, the latter built for GPUs) does not depend on evaluation order, so any split of the work across cores or devices returns the same bits; the fixed order it replaces could only be reproduced by one thread per node. Tensors carry their device, as in PyTorch and JAX, so one tape differentiates across devices instead of a second tape bridged to the first.
  timestamp: 2026-09-25
  impacted_ticket_ids: M33–M39

- decision: A device `EdgeSum` kernel is not wired. On the Apple M3 the device matches the CPU's contracted order bit for bit, but it beats the all-core CPU only on the largest graphs, and by at most 1.8×; whether that earns a production kernel, and whether resident tensors are pursued, goes to the owner.
  rationale: `measure-edge-sum-device`, 2026-09-25, 8-core M3, Metal, best of 5 after a warm-up call, random sources and targets, two runs agreeing within 10–20%. Parity: Metal fuses `acc + w*v` like Go on arm64 (0 mismatches against the fused loop, 99,508 of 200,000 against the contract on a 100k-node, 1M-edge graph); the kernel that passes the product through an xor with a runtime zero matches the contracted order on every output of every graph, and every timed device result equals `nn.EdgeSum` bit for bit. Speed, device against all-core CPU (second run): below 100M edge·batch products the device loses, 0.10–0.54× uploading everything per call and 0.17–0.53× with the topology resident (10k nodes, 10 edges each, B=1: CPU 0.10 ms, device 0.58 ms); at 100k nodes, 100 edges, B=1 it breaks even (1.07× resident); it wins only at 160M products, 1.46× and 1.61× at 100k nodes, 100 edges, B=16 (CPU 424 ms, device 263 ms resident) and 1.67× and 1.79× at 1M nodes, 10 edges, B=16 (CPU 637 ms, device 355 ms resident). Keeping the topology resident gains at most 0.2×, so transfers are not what limits the device here: on unified memory the GPU gathers from the same memory as the CPU, and the operation is bound by that memory. A discrete GPU with its own memory bandwidth could differ, and this host cannot measure it.
  timestamp: 2026-09-25
  impacted_ticket_ids: measure-edge-sum-device; #379 steps 2c, 3 and 4
