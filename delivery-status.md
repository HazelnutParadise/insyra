...
## Next Ticket
#379 waits on the owner. The device measurement (`measure-edge-sum-device`) found a device `EdgeSum` bit-identical to the CPU but at most 1.8× faster than all cores, and only on the largest graphs; the Decision Log has the numbers. Open: whether that earns a production kernel behind a size floor, whether resident tensors (step 2c) are pursued given that transfers are not the limit on unified memory, and what step 4, state export and import, should cover if there is no device path.
