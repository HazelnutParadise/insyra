...
Also awaiting a ruling: `CSVWriteOptions.SanitizeFormulas`'s default (commented on #285), what to do with two unmerged legacy branches (`copilot/fix-aa06638c-…`, `lingo-to-lp`), the `AGENTS.md` follow-up proposing a nil-receiver guard on `Err()`, and the two CLI follow-ups found on 2026-09-11 (`count`/`find`/`replace` never match an integer; arguments that are accepted and ignored, `accel --precision` among them).
