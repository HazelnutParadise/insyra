...
`parquet-stream-error-not-lost` — the streaming read's error channel and record channel are closed together, so the `select` in `FilterWithCCL`, `ApplyCCL` and `Stream` can pick either and drop an error that arrives after the last batch. Found and proven on 2026-09-12; the fix is local and non-breaking.

After that, owner decision needed: pick from the decision-fork list above. Most of the remaining `severity:med` issues are API redesigns that need the same kind of decision. The decision-free ones left are the rest of the test coverage work (#307 gplot, #309 the sixteen untested packages) and the CLI documentation mismatches (#321).
