...
## Handoff Notes
- **Two sample-weight design questions are open, deliberately.** WLS landed because its math is closed-form and reference-checkable. Tree sample weights would push float weights into histogram accumulators the precision contract fixed as integers for associativity — an architecture decision against ENG.md, not a feature. And weights flowing through cross-validation need a channel in `Estimator.Fit(x, y)` and the metric protocol — a breaking protocol change. Neither should be decided by whoever happens to implement next.
