...
- decision: `nn.Tanh` is correctly rounded on every platform (M34), computed the way CORE-MATH and RLIBM compute binary32 functions. `|x| < 2^-13` and `|x| >= 9.5` are settled by proof. Between them a float64 approximation of our own, `tanhFloat64`, uses only IEEE 754 `+ − × ÷` with every product rounded by an explicit conversion, so it returns the same bits on every platform. Its float32 rounding is accepted when it lies more than 16 float64 steps from every float32 midpoint. The four inputs inside that margin are a table, and `math/big` stays behind the table. `Tape.Tanh`'s gradient rounds every step and fuses none.
