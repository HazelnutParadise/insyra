...
| M33 | Edge sums are exact | planning | pending | `EdgeSum` and both gradients return the correctly rounded exact sum of their products, equal to a `math/big` oracle on random, cancelling, subnormal, overflowing, NaN/Inf and signed-zero inputs, with the CPU cost against the previous fixed-order version measured |
...
## Next Ticket
`exact-edge-sum` (M33): `EdgeSum` and both gradients return the correctly rounded exact sum of their products. Then M34 through M39 in order; the Decision Log entry of 2026-09-25 gives the reasons.
