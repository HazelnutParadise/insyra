...
## Current Phase
Production-readiness hardening (2026-09-06). Every package's exported surface was reviewed symbol by symbol, then the whole repository was reviewed beyond exported symbols (CLI command behaviour, CCL semantics, internal packages, security, test quality, repo/CI hygiene). The ledger is `api-review.md`; every open finding is a GitHub issue labelled `api-review` with a `severity:*` label (#205–#368). Fixes land in OpenSpec batches: `fix-api-review-batch-1` through `-8`, then changes named for what they fix (listed under Latest Milestones). 98 findings remain open (7 high, 42 medium, 49 low), counted from the `api-review` label rather than from arithmetic on this file — closing four medium issues moved the medium count by five, the same off-by-one this ledger hit on 2026-09-12, and the tracker is the authority.
...
Owner decision needed: pick from the decision-fork list above. Most of the remaining `severity:med` issues are API redesigns that need the same kind of decision. The decision-free ones left are the rest of the test coverage work (#307 gplot, #309 the sixteen untested packages) and the CLI documentation mismatches (#321).
