...
| M34 | `Tanh` is correctly rounded | planning | pending | `nn.Tanh` equals the correctly rounded `tanh` on all 2^32 float32 inputs, checked exhaustively against a high-precision oracle, and the number of inputs that changed is recorded |
...
## Next Ticket
`correctly-rounded-tanh` (M34): `nn.Tanh` equals the correctly rounded `tanh` on all 2^32 float32 inputs, checked exhaustively, with the number of inputs that change recorded. Then M35 through M39 in order; M33 is done.
