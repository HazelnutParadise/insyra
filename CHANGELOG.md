...
- Added `accel.OpNearestShortlist` and `Session.ExecuteNearestExact`, which returns the M nearest query points per row as exact `float64` values while still using a GPU for most of the work. The device ranks in single precision and returns a shortlist per row plus the distance of the best candidate it discarded; the host recomputes that shortlist in `float64` and decides, falling back to every query point for the rows where single precision could not separate the candidates. The answer equals `accel.NearestExactCPU` exactly, so no precision opt-in is required. Measured on an Apple M3 over 200,000 rows by 16 dimensions asking for the two nearest: 3.2x at 64 query points and 7.1x at 256, with no rows needing a full recompute. Below 32 query points the round trip costs more than it saves, so the runtime declines the device and reports `workload-not-profitable`. `ExactNearestResult.Rechecked` reports how many rows took the full path.
