...
- Added the separate `github.com/HazelnutParadise/insyra/accel/backend/wgpu` module, a pure-Go GPU backend built on [gogpu/wgpu](https://github.com/gogpu/wgpu). Import it for its side effect to make GPU execution available. It builds with `CGO_ENABLED=0` and reaches Metal on macOS, Vulkan on Linux and Windows, and DirectX 12 on Windows. The core `insyra` module gains no GPU dependency.
