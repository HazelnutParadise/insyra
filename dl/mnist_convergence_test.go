...
func TestMNISTConvergence(t *testing.T) {
	root, ok := os.LookupEnv("INSYRA_DL_MNIST_DIR")
	if !ok || root == "" {
		t.Skip("INSYRA_DL_MNIST_DIR is unset")
	}
	info, err := os.Stat(root)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			t.Skipf("INSYRA_DL_MNIST_DIR=%q is missing", root)
		}
		t.Fatalf("stat INSYRA_DL_MNIST_DIR=%q: %v", root, err)
	}
	if !info.IsDir() {
		t.Fatalf("INSYRA_DL_MNIST_DIR=%q is not a directory", root)
	}
...
func mnistReadFailure(t *testing.T, root string, err error) {
	t.Helper()
	if errors.Is(err, fs.ErrNotExist) {
		t.Skipf("INSYRA_DL_MNIST_DIR=%q is missing a required IDX file: %v", root, err)
	}
	t.Fatalf("read MNIST from INSYRA_DL_MNIST_DIR=%q: %v", root, err)
}
