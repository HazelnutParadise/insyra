...
func requireAutodiffReference(t *testing.T) string {
	...
	if err != nil {
		reftest.Missing(t, "python3", "the dl PyTorch autodiff parity check", err)
		return ""
	}
	...
	command.Stderr = &stderr
	if err := command.Run(); err != nil {
		reftest.MissingOutput(t, "python3 with torch and safetensors", "the dl PyTorch autodiff parity check", err, stderr.Bytes())
		return ""
	}
	return python
}
