...
func TestRealModelParity(t *testing.T) {
	modelDir := os.Getenv("INSYRA_DL_REAL_MODELS_DIR")
	if modelDir == "" {
		t.Skip("INSYRA_DL_REAL_MODELS_DIR is not set")
	}
	models := []struct {
		name string
		file string
	...
	for _, tc := range models {
		if _, err := os.Stat(filepath.Join(modelDir, tc.file)); err != nil {
			t.Skipf("INSYRA_DL_REAL_MODELS_DIR is missing %s: %v", tc.file, err)
		}
	}
	...
	for _, tc := range models {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("run %s in dl: %v", tc.name, err)
			}
			...
			for index, spec := range model.Outputs() {
				...
				if !present {
					t.Fatalf("dl output %q is missing", spec.Name)
				}
