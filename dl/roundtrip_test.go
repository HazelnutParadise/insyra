...
// and run in dl, reproducing the fitted model's own predictions. This half of
...
// onnxruntime, and dl's parity harness pins every kernel against it, so the
...
func exportAndLoad(t *testing.T, model ml.Model) *dl.Model {
	t.Helper()
	...
	loaded, err := dl.LoadONNX(&buffer)
	if err != nil {
		t.Fatalf("dl refused ml's own export: %v", err)
	}
	return loaded
}
...
func runExportedGraph(t *testing.T, model *dl.Model, features *insyra.DataTable) map[string]*dl.Tensor {
	t.Helper()
	inputs := make(map[string]*dl.Tensor, len(model.Inputs()))
	for index, info := range model.Inputs() {
		...
		tensor, err := dl.NewTensor([]int{column.Len()}, values)
		if err != nil {
			t.Fatal(err)
		}
...
// compareLists checks a dl output tensor against ml's own DataList prediction.
// tolerance 0 means exact (labels); otherwise absolute-with-scale for the f32
// round trip through the exchange format.
func compareLists(t *testing.T, name string, got *dl.Tensor, want *insyra.DataList, tolerance float64) {
	t.Helper()
	if got == nil {
