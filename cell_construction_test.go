...
func TestEveryEntryPointUnwrapsTheMark(t *testing.T) {
	...
	// The replace-with and fill entry points this line has on top of 0.4's
	// list: each stores a caller's value in a cell, so each has to unwrap it.
	...
	t.Run("DataTable.Replace", func(t *testing.T) {
		dt := NewDataTable(NewDataList(1).SetName("c"))
		dt.Replace(1, Cell(v))
		assertHolds(t, dt.GetElementByNumberIndex(0, 0), v)
	})
	t.Run("DataTable.ReplaceNaNsAndNilsWith", func(t *testing.T) {
		dt := NewDataTable(NewDataList(nil).SetName("c"))
		dt.ReplaceNaNsAndNilsWith(Cell(v))
		assertHolds(t, dt.GetElementByNumberIndex(0, 0), v)
	})
	t.Run("DataTable.ReplaceInRow", func(t *testing.T) {
		dt := NewDataTable(NewDataList(1).SetName("c"))
		dt.ReplaceInRow(0, 1, Cell(v))
		assertHolds(t, dt.GetElementByNumberIndex(0, 0), v)
	})
	t.Run("DataTable.ReplaceNaNsAndNilsInRow", func(t *testing.T) {
		dt := NewDataTable(NewDataList(nil).SetName("c"))
		dt.ReplaceNaNsAndNilsInRow(0, Cell(v))
		assertHolds(t, dt.GetElementByNumberIndex(0, 0), v)
	})
	t.Run("DataTable.ReplaceInCol", func(t *testing.T) {
		dt := NewDataTable(NewDataList(1).SetName("c"))
		dt.ReplaceInCol("A", 1, Cell(v), 1)
		assertHolds(t, dt.GetElementByNumberIndex(0, 0), v)
	})
	t.Run("DataTable.ReplaceNaNsAndNilsInCol first", func(t *testing.T) {
		dt := NewDataTable(NewDataList(nil).SetName("c"))
		dt.ReplaceNaNsAndNilsInCol("A", Cell(v), 1)
		assertHolds(t, dt.GetElementByNumberIndex(0, 0), v)
	})
}
