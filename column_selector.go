...
func (dt *DataTable) lookupColSelector(selector any) (num int, warning, problem string) {
	...
	case string:
		return dt.lookupColIndexString(v)
	case NameSelector:
		position, ok := dt.getColNumberByName_notAtomic(v.value)
		if !ok {
	...
	case int:
		position := v
		if position < 0 {
			position = len(dt.columns) + position
		}
		if position < 0 || position >= len(dt.columns) {
			return -1, "", fmt.Sprintf("column number %d is out of range, the table has %d column(s)", v, len(dt.columns))
		}
		return position, "", ""
	default:
...
// lookupColIndexString resolves a bare string, which is always an Excel-style
...
// means.
func (dt *DataTable) lookupColIndexString(index string) (num int, warning, problem string) {
	position, ok := utils.ParseColIndex(index)
	if !ok {
		if _, named := dt.getColNumberByName_notAtomic(index); named {
	...
	if position >= len(dt.columns) {
		letters, _ := utils.CalcColIndex(position)
		if _, named := dt.getColNumberByName_notAtomic(index); named {
			return -1, "", fmt.Sprintf("%q reads as column index %s, past the last of %d column(s), and this table has a column named %s, so write Name(%q)", index, letters, len(dt.columns), index, index)
		}
		return -1, "", fmt.Sprintf("column %s does not exist, the table has %d column(s)", letters, len(dt.columns))
	}
	if named, ok := dt.getColNumberByName_notAtomic(index); ok && named != position {
		letters, _ := utils.CalcColIndex(position)
