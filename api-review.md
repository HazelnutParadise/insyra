...
| SEC-8 | Med（併入 K-1） | 函式庫路徑內 `os.Exit`（K-1 的新呼叫點清單）：GLPK 下載／解壓／編譯失敗 `lp/init.go:151,156,162,195,203,211,219`、Windows 初始化 `:127`、`SolveModel` 建暫存檔失敗 `lp/lp.go:143,150`、py IPC socket 監聽失敗 `py/pyresult.go:88` | logger.go:20-24 及上述 | 改回傳 error；併入 K-1 |
