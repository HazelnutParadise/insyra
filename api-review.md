...
| CCL-15 | Med（錯誤已帶語句原文與列號，batch 9；半改狀態與每語句 snapshot 待 E-2） | E-2 補充：`ExecuteCCL("NEW('x') = A + 1; NEW('y') = A / 0; NEW('z') = A")` 後表有 `x` 沒有 `y`、`z`，`Err()` 只有 `division by zero` 不說第幾條；每條語句後都整表 copy 一次 snapshot | datatable_ccl.go:179-197 | 先全部 Bind 再執行；錯誤帶語句序號與原文（併入 E-2） |
